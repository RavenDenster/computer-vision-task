...
while True:
    ...
    fgmask = knn.apply(frame)  # Разделите фон

    th = cv.threshold(fgmask.copy(), 254, 255, cv.THRESH_BINARY)[1]

    dilated = cv.dilate(th, cv.getStructuringElement(cv.MORPH_ELLIPSE, (3, 3)), iterations=1)

    contours, hier = cv.findContours(dilated, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    for c in contours:
        if cv.contourArea(c) > 2500:
            epsilon = 0.0002 * cv.arcLength(c, True)
    ...
    for (classid, score, box) in zip(classes, scores, boxes):
        ...
        inverted_contour = cv.bitwise_not(frame[box[1]-20:box[1]-4, box[0]-2:box[0]+120])
    ...
    # Предполагается, что у вас есть переменная "frame", содержащая изображение с выделенным объектом
